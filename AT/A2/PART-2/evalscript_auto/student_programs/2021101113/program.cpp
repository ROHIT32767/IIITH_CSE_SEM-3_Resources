...
int main(void)
{
    ...
    if (!thousand)
    {
        ...
        {
            if ((node[i].state_type != 'R') && (node[i].state_type != 'S') && (node[i].state_type != 'P'))
            {
                node[i].state_type = 'R';
            }
            if (index_of_encodings[node[i].on_rock] == 0)
            {
                index_of_encodings[node[i].on_rock] = (rand() % (final_index + 1)) + 1;
            }
            if (index_of_encodings[node[i].on_scissors] == 0)
            {
                index_of_encodings[node[i].on_scissors] = (rand() % (final_index + 1)) + 1;
            }
            if (index_of_encodings[node[i].on_paper] == 0)
            {
                index_of_encodings[node[i].on_paper] = (rand() % (final_index + 1)) + 1;
            }
    ...
    else
    {
        ...
        for (INT i = 0; i <= 999; i++)
        {
            ...
            if (index_of_encodings[node[i].on_rock] == 0)
            {
                index_of_encodings[node[i].on_rock] = (rand() % 1000) + 1;
            }
            if (index_of_encodings[node[i].on_scissors] == 0)
            {
                index_of_encodings[node[i].on_scissors] = (rand() % 1000) + 1;
            }
            if (index_of_encodings[node[i].on_paper] == 0)
            {
                index_of_encodings[node[i].on_paper] = (rand() % 1000) + 1;
            }
            cout << node[i].state_type << " " << index_of_encodings[node[i].on_rock] << " " << index_of_encodings[node[i].on_paper] << " " << index_of_encodings[node[i].on_scissors] << endl;
        }
    }
